...
class Farina:
    ...
    def getTHD(self, harms=9):
        ...
        """
        # arranquem desde l'harmonic 1, no el 0 que seria el fonamental
        rms_vals = np.zeros(harms+1)
        r_corr = signal.convolve(self.get_IR(), self.get_IR())
        rms_vals[0] = np.sqrt(np.mean(r_corr**2))
        ...
        for idx, response in enumerate(self.harm_responses[:harms]):
            ...
            # rms_vals[0] /= rms_vals[0] # normalitzem el fonamental a 1
        return (np.sqrt(np.sum(rms_vals[1:]**2)) / rms_vals[0])*100
